	...
	namespace interface
	{
		...
		void Engine::start(void)
		{
			...
			while(!glfwWindowShouldClose(m_window))
			{
				...
				m_ubo->transfer(sizeof(Screen), sizeof(Camera), &m_camera);
		...
		void Engine::setup_scene(void)
		{
			...
			m_ubo->allocate(sizeof(Screen) + sizeof(Camera) + sizeof(RayMarcher));
			//ubo transfer
			m_ubo->transfer(0, sizeof(Screen), &m_screen);
			m_ubo->transfer(sizeof(Screen), sizeof(Camera), &m_camera);
			m_ubo->transfer(sizeof(Screen) + sizeof(Camera), sizeof(RayMarcher), &m_ray_marcher);
		}
